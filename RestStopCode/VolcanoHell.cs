...
{
	public class VolcanoDungeonHell : IslandLocation
	{
		...
		public static List<VolcanoDungeonHell> activeLevels = new List<VolcanoDungeonHell>();
		...
		public VolcanoDungeonHell()
		{
			mapContent = Game1.game1.xTileContent.CreateTemporary();
			mapPath.Value = "Maps\\Mines\\VolcanoTemplate";
		}

		public VolcanoDungeonHell(int level): this()
		{
			this.level.Value = level;
			name.Value = "VolcanoDungeonHell" + level;
		}

		internal static string BaseLocationName = "VolcanoDungeonHell";

		//Borrowed Code from KittyCatCasey

		public static VolcanoDungeonHell GetLevelInstance(string locName)
		{
			...
			int l = int.Parse(locName.Substring(BaseLocationName.Length));
			VolcanoDungeonHell newLevel = new VolcanoDungeonHell(l);
			activeLevels.Add(newLevel);

			if (Game1.IsMasterGame)
				newLevel.GenerateContents();
			else
				newLevel.reloadMap();
		...
		{
			base.initNetFields();
			base.NetFields.AddFields(level, coolLavaEvent, cooledLavaTiles.NetFields, generationSeed, layoutIndex);
		...
		public virtual void GenerateEntities()
		{
			...
			if ((int)level > 0 && (int)level != 5 && (generationRandom.NextDouble() < 0.5 ))
			{
		...
		public static VolcanoDungeonHell GetLevel(string name, bool use_level_level_as_layout = false)
		{
			foreach (VolcanoDungeonHell level in activeLevels)
			{
			...
			VolcanoDungeonHell new_level = new VolcanoDungeonHell(Convert.ToInt32(name.Substring("VolcanoDungeonHell".Length)));
		...
		public static void ForEach(Action<VolcanoDungeonHell> action)
		{
			foreach (VolcanoDungeonHell level in activeLevels)
			{
				action(level);
